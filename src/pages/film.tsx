    ...
    return (
        <>
            ...
            <div className='h-[300px] left-0 right-0 top-0 relative'>
                ...
                <Image src={tmdbImageSrc(film.coverPath)} className='rounded-0 rounded-none'></Image>
            </div>
            {/* poster and text */}
            <Section className='-mt-[150px] flex items-center relative z-10 mobile:block'>
                <Image src={tmdbImageSrc(film.posterPath)} className='max-h-[400px] max-w-[200px] mobile:mx-auto'></Image>
                    ...
                    <div className='flex items-center gap-3'>
                        {casts.map((cast, i) => (
                            <div className='flex-shrink-0 w-[200px] mb-6' key={i}>
                                <Card withPlay={false} imageSrc={tmdbImageSrc(cast.profilePath)}>
                ...
                <div className='overflow-x-auto scrollbar scrollbar-thumb-primary scrollbar-track-header'>
                    <div className='flex items-center gap-3 h-[300px]'>
                        {trailers.map((trailer, i) => (
                            <Card onClick={() => playTrailer(trailer.key)} imageSrc={youtubeThumbnail(trailer.key)} className='flex-shrink-0' key={i}></Card>
                        ))}
                    </div>
                </div>
            </Section>
            ...
            <Section title='Seasons' hidden={film.seasons.length === 0}>
                <Slider slidesToShow={film.seasons.length > 2 ? 2 : 1} slidesToScroll={film.seasons.length > 2 ? 2 : 1} swipe={false}>
                    {(_) =>
            ...
            <Section title='Recommendations' hidden={recommendations.length === 0}>
                <Slider isMovieCard={true}>
                    {(_) =>
                        recommendations.map((film, i) => <Card onClick={() => navigate(`/${props.mediaType}/${film.id}`)} title={film.title} imageSrc={tmdbImageSrc(film.posterPath)} key={i}></Card>)
                    }
                </Slider>
            </Section>
        </>
