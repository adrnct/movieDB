...
export const Catalog = (props: Props) => {
    ...
    return (
        <>
            ...
            {/* PAGE TITLE */}
            <Section className='-mt-[90px] flex items-center relative z-10' title={title}></Section>
            {/* Films */}
            <Section>
                <div className='grid lg:grid-cols-5 sm:grid-cols-4 mobile:grid-cols-3 relative z-[11]'>
                    {films.map((film, i) => (
                        <div key={i}>
                            <Card onClick={() => navigate(`/${film.mediaType}/${film.id}`)} imageSrc={tmdbImageSrc(film.posterPath)} title={film.title} key={i}></Card>
                        </div>
                    ))}
                </div>
            </Section>
