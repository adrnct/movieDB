        ...
        <>
                ...
                <Slider className='slick-hero' autoplay={true} slidesToShow={1} slidesToScroll={1}>
                    {(onSwipe) =>
                        trendings.map((film, i) => (
                            <TrendingHero onPlayTrailer={() => playTrailer(film)} onClick={() => (!onSwipe ? navigate(`/${film.mediaType}/${film.id}`) : '')} film={film} key={i}></TrendingHero>
                        ))
                    }
                </Slider>
            </Section>
            ...
            <Section title='In Theaters' hidden={inTheaters.length === 0}>
                <Slider isMovieCard={true}>
                    {(_) => inTheaters.map((film, i) => <Card onClick={() => goToDetailPage(film)} title={film.title} imageSrc={tmdbImageSrc(film.posterPath)} key={i}></Card>)}
                </Slider>
            </Section>
            {/* populars */}
            <Section title="What's Popular" hidden={populars.length === 0}>
                <Slider isMovieCard={true}>
                    {(_) => populars.map((film, i) => <Card onClick={() => goToDetailPage(film)} title={film.title} imageSrc={tmdbImageSrc(film.posterPath)} key={i}></Card>)}
                </Slider>
            </Section>
            {/* top rated tv */}
            <Section title='Top Rated TV' hidden={topRatedTv.length === 0} onTitleClick={() => navigate(`/list/top-rated-tv`)}>
                <Slider isMovieCard={true}>
                    {(_) => topRatedTv.map((film, i) => <Card onClick={() => goToDetailPage(film)} title={film.title} imageSrc={tmdbImageSrc(film.posterPath)} key={i}></Card>)}
                </Slider>
            </Section>
            {/* to rated movies*/}
            <Section hidden={topRatedMovie.length === 0} title='Top Rated Movies' onTitleClick={() => navigate(`/list/top-rated-movies`)}>
                <Slider isMovieCard={true}>
                    {(_) => topRatedMovie.map((film, i) => <Card onClick={() => goToDetailPage(film)} title={film.title} imageSrc={tmdbImageSrc(film.posterPath)} key={i}></Card>)}
                </Slider>
            </Section>
        </>
    )
